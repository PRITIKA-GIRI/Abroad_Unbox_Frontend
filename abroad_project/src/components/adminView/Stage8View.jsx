...
const Stage8View = () => {
  ...
  const getStage8Data = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/stage-eight-submissions/`);
      setStage8Data(response.data.results || []);
    } catch (error) {
  ...
  const handlePopup = async (studentId) => {
    try {
      const res = await axios.get(
        `${API_BASE_URL}/stage-eight-submissions/?student=${studentId}`
      );
  ...
  const handleApprove = async (studentId, stage, submissionId) => {
    try {
      ...
      await axios.patch(
        `${API_BASE_URL}/stage-eight-submissions/${submissionId}/`,
        { status: "completed" }
      );
      alert("User stage 8 approved");
      closePopup();
      getStage8Data();
    } catch (error) {
  ...
  const handleDecline = async (submissionId) => {
    if (!window.confirm("Are you sure you want to decline stage 8?")) return;
    try {
      await axios.delete(
        `${API_BASE_URL}/stage-eight-submissions/${submissionId}/`
      );
      alert("Stage 8 declined");
      closePopup();
      getStage8Data();
    } catch (error) {
      console.error("Failed to delete the stage 8 submission", error);
    }
  };
  ...
  return (
    <>
      ...
      <div className="w-11/12 p-5 mx-auto mt-5">
        <h2 className="text-center text-3xl font-bold">Stage 8 Submissions</h2>
      ...
      {showPopup && selectedStudentData && (
        ...
        >
          ...
          >
            ...
            <div className="mb-6">
              <h4 className="text-xl font-bold mb-2">Applied Session</h4>
              {selectedStudentData.applied_session?.map((session, idx) => (
                <div
                  key={idx}
                  className="p-3 border border-gray-300 rounded-lg mb-3 bg-gray-50"
                >
                  <p className="font-semibold mb-2">Session #{idx + 1}:</p>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <span className="font-semibold">ID:</span> <span>{session.id}</span>
                    </div>
                    <div>
                      <span className="font-semibold">Date & Time:</span>{" "}
                      <span>{session.display_range}</span>
                    </div>
                  </div>
                </div>
              ))}
