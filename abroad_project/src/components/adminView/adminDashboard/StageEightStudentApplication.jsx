...
export default function StageEightStudentApplication({ studentId, onClose }){
  const [studentStageEightDetail, setStudentStageEightDetail] = useState([]);
  const [loading, setLoading] = useState(true);

  const fetchStudentStageEightDetail = async () => {
    setLoading(true);
    try {
      const { data } = await axios.get(
        `${API_BASE_URL}/stage-eight-submissions/?student=${studentId}`
      );
      setStudentStageEightDetail(data.results);
    } catch (error) {
      console.error("Error fetching student stage eight detail:", error);
      setStudentStageEightDetail([]);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const handleApprove = async (stage, submissionId) => {
        try {
            ...
            alert("Student's stage 8 approved!");
            fetchStudentStageEightDetail();
        } catch (err) {
    ...
    const handleDecline = async (submissionId) => {
        ...
        try {
            ...
            alert("Student's stage 8 declined.");
            fetchStudentStageEightDetail();
        } catch (err) {
        ...
        return (
            <div className="flex items-center justify-center h-auto bg-gray-100 w-full p-5">
                <p className="text-gray-500 text-sm">
                    Loading student's stage eight details...
                </p>
            </div>
        );
    }
        ...
        <div className="flex flex-col items-center justify-center h-auto bg-gray-100 w-full">
            <div className="bg-white shadow-md rounded-lg px-6 py-3 w-full max-w-4xl">
                <h2 className="text-2xl font-semibold mb-4 text-center">
                    Stage Eight Student Application
                </h2>

                {studentStageEightDetail.length > 0 ? (
                    studentStageEightDetail.map((detail) => (
                        <div
                        key={detail.id}
                        ...
                        >
                            ...
                            <div className="col-span-2"><strong>Applied Session: </strong> <span>{detail.applied_session[0]?.display_range || "—"}</span></div>
