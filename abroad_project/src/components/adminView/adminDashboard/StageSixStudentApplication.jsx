...
export default function StageSixStudentApplication({ studentId, onClose }){
  const [studentStageSixDetail, setStudentStageSixDetail] = useState([]);
  const [stdUniEssayDetail, setStdUniEssayDetail] = useState([]);
  const [loading, setLoading] = useState(true);

  const fetchStudentStageSixDetail = async () => {
    setLoading(true);
    try {
      const { data } = await axios.get(
        `${API_BASE_URL}/stage-six-submissions/?student=${studentId}`
      );
      setStudentStageSixDetail(data.results);
    } catch (error) {
      console.error("Error fetching student stage six detail:", error);
      setStudentStageSixDetail([]);
    } finally {
      setLoading(false);
    }
  };

  const fetchStdUniEssayDetail = async () => {
    setLoading(true);
    try {
        const { data } = await axios.get(
            `${API_BASE_URL}/student-university-essay-details/?student=${studentId}`
        );
        setStdUniEssayDetail(data);
    } catch (error) {
        console.error("Erro Fetchin essay detail:", error);
        setStdUniEssayDetail([]);
    } finally {
        setLoading(false);
    }
  }
  ...
  useEffect(() => {
    if (studentId) {
      fetchStudentStageSixDetail();
      fetchStdUniEssayDetail();
    }
  }, [studentId]);
  ...
  const handleApprove = async (stage, submissionId) => {
    try {
      ...
      alert("Student's stage 6 approved!");
      fetchStudentStageSixDetail();
    } catch (err) {
  ...
  const handleDecline = async (submissionId) => {
    ...
    try {
        ...
        alert("Student's stage 6 declined.");
        fetchStudentStageSixDetail();
    } catch (err) {
        ...
        return (
            <div className="flex items-center justify-center h-auto bg-gray-100 w-full p-5">
                <p className="text-gray-500 text-sm">
                    Loading student's stage six details...
                </p>
            </div>
        );
    }
        ...
        <div className="flex flex-col items-center justify-center h-auto bg-gray-100 w-full">
            <div className="bg-white shadow-md rounded-lg px-6 py-3 w-full max-w-4xl">
                <h2 className="text-2xl font-semibold mb-4 text-center">
                    Stage Six Student Application
                </h2>

                {studentStageSixDetail.length > 0 ? (
                    studentStageSixDetail.map((detail) => (
                        <div
                        key={detail.id}
                        ...
                        >
                            ...
                            <span className="text-xl font-bold mb-3">Applied Universities Essay Detail:</span>
                            {stdUniEssayDetail.map((uni_essay_detail, i) => (
                                <div key={uni_essay_detail.id}>
                                    <div className="bg-white p-2 rounded-lg mt-2 flex flex-col items- center cursor-pointer">
                                        <div><strong>University: </strong><span> {uni_essay_detail.university_name}</span></div>
                                        <div><strong>Essay Title: </strong> <span>{uni_essay_detail.title}</span></div>
                                        <a href={uni_essay_detail.essay_file} target="_blank" className="text-blue-500 hover:text-blue-600 hover:underline">View File</a>
                                    </div>
