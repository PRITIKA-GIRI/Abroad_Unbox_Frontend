...
export default function StageFourStudentApplication({ studentId, onClose }) {
  const [studentStageFourDetail, setStudentStageFourDetail] = useState(null);
  const [loading, setLoading] = useState(true);

  const fetchStudentStageFourDetail = async () => {
    setLoading(true);
    try {
      const { data } = await axios.get(
        `${API_BASE_URL}/stage-four-submissions/?student=${studentId}`
      );
      setStudentStageFourDetail(data.results);
    } catch (error) {
      console.error("Error fetching student stage four detail:", error);
      setStudentStageFourDetail([]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApprove = async (stage, submissionId) => {
    try {
      ...
      await axios.patch(
        `${API_BASE_URL}/stage-four-submissions/${submissionId}/`,
        { status: "completed" }
      );
      alert("Student's stage 4 approved!");
      fetchStudentStageFourDetail();
    } catch (err) {
  ...
  const handleDecline = async (submissionId) => {
    ...
    try {
        ...
        alert("Student's stage 4 declined.");
        fetchStudentStageFourDetail();
    } catch (err) {
  ...
  if (loading) {
        ...
        <div className="flex items-center justify-center h-auto bg-gray-100 w-full p-5">
            ...
            Loading student's stage four details...
            </p>
        </div>
        );
    }
        ...
        <div className="flex flex-col items-center justify-center h-auto bg-gray-100 w-full">
            <div className="bg-white shadow-md rounded-lg px-6 py-3 w-full max-w-4xl">
                <h2 className="text-2xl font-semibold mb-4 text-center">
                    Stage Four Student Application
                </h2>

                {studentStageFourDetail.length > 0 ? (
                studentStageFourDetail.map((detail) => (
                    <div
                    key={detail.id}
                    ...
                    >
                        ...
                        <div className="flex gap-3"><strong>Resume: </strong> {detail.resume ? (  <a href={detail.resume} target="_blank" className="text-blue-500 hover:underline hover:text-blue-600">View File</a> ): ( <span>N/A</span> )} </div> 
                        ...
                        {/* <div><span className="text-xl font-bold">Extracurricular Activities Detail:</span></div> */}
                        ...
                        {detail.eca_data.map((act, i) => (
                            <div key={act.i}>
                                <div className="flex flex-col gap-2">
                                    <div className="bg-white p-2 rounded-lg mt-2">
                                      <strong>ECA {i+1}:</strong><br/>
                                      <strong>Activity Type: </strong><span>{act.activities_type}</span><br/>
                                      <strong>Position: </strong><span> {act.position} </span><br/>
                                      <strong>Organization: </strong><span>{act.org_involved}</span><br/>
                                      <strong>Roles and Duties: </strong><span>{act.roles_duties}</span>
                                    </div>
                                </div>
                            </div>
                        ))}
