...
export default function StageSevenStudentApplication({ studentId, onClose }){
  const [studentStageSevenDetail, setStudentStageSevenDetail] = useState([]);
  const [loading, setLoading] = useState(true);

  const fetchStudentStageSevenDetail = async () => {
    setLoading(true);
    try {
      const { data } = await axios.get(
        `${API_BASE_URL}/stage-seven-submissions/?student=${studentId}`
      );
      setStudentStageSevenDetail(data.results);
    } catch (error) {
      console.error("Error fetching student stage seven detail:", error);
      setStudentStageSevenDetail([]);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const handleApprove = async (stage, submissionId) => {
        try {
            ...
            alert("Student's stage 7 approved!");
            fetchStudentStageSevenDetail();
        } catch (err) {
    ...
    const handleDecline = async (submissionId) => {
        ...
        try {
            ...
            alert("Student's stage 7 declined.");
            fetchStudentStageSevenDetail();
        } catch (err) {
        ...
        return (
            <div className="flex items-center justify-center h-auto bg-gray-100 w-full p-5">
                <p className="text-gray-500 text-sm">
                    Loading student's stage seven details...
                </p>
            </div>
        );
    }
        ...
        <div className="flex flex-col items-center justify-center h-auto bg-gray-100 w-full">
            <div className="bg-white shadow-md rounded-lg px-6 py-3 w-full max-w-4xl">
                <h2 className="text-2xl font-semibold mb-4 text-center">
                    Stage Seven Student Application
                </h2>

                {studentStageSevenDetail.length > 0 ? (
                    studentStageSevenDetail.map((detail) => (
                        <div
                        key={detail.id}
